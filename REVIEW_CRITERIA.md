...
- the onPage and onFailure callbacks are not asserted in crawler.test.ts — the failure result is checked via result.failures but the callbacks themselves are never verified to fire. this is an orchestration-layer gap because the decision of when to call them, with what data, and in what order is wiring logic inside crawler.ts that no unit test touches. should be a small addition to the existing failure test rather than a new test.
